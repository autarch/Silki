...
after '_set_user' => sub
{
    ...
    $c->add_tab( { uri => $self->_make_user_uri( $c, $user ),
                   id  => 'profile',
                   %profile,
                 }
               );
    ...
    if ( $c->user()->can_edit_user($user) )
    {
        ...
        $c->add_tab( { uri => $self->_make_user_uri( $c, $user, 'preferences_form' ),
                       id  => 'preferences',
                       %prefs,
                     }
                   );
...
sub user_GET_html
{
    ...
    unless ( $c->user()->is_system_user() )
    {
        $c->stash()->{shared_wikis} =
            $c->stash()->{user}->wikis_shared_with( $c->user() );
    }
...
sub _user_update_error
{
    ...
    $c->redirect_with_error
        ( error     => $errors,
          uri       => $self->_make_user_uri( $c, $c->stash()->{user}, 'preferences_form' ),
          form_data => $form_data,
        );
}
